...
function login(username,password,loginCaptcha) {
	...
	.done(function(json){
		...
		//登陆失败
			...
			if(json.msg==='验证码错误'){
				$("#captcha_error_login").show();
				$("#captcha_error_login").html("验证码错误！"); 
			}else if(json.msg==='用户名或密码错误'){
				$("#password_error_login").show();
				$("#password_error_login").html("用户名或密码错误!");
			}else if(json.msg==='该用户不存在'){
				$("#username_error_login").show();
				$("#username_error_login").html("该用户不存在!"); 
			}else{
...
function register(username,nickname,password,certainPassword,regCaptcha) {
	...
	.done(function(json){
		...
		//注册失败
			...
			if(json.msg==='验证码错误！'){
				$("#captcha_error_reg").html("验证码错误！"); 
				$("#captcha_error_reg").show();
			}else if(json.msg==='该用户名已存在'){
				$("#username_error_reg").html("该用户名已存在"); 
				$("#username_error_reg").show();
			}else if(json.msg==='该邮箱已存在'){
...
function addHandler(id, action, func) {
 ...
 function eventInit() {
    ...
    //登录的输入事件
    addHandler('username_login','change',checkLoginUsername);
    addHandler('password_login','change',checkLoginPassword);
    addHandler('captcha_login','change',checkLoginCaptcha);
}
 eventInit();
